...
const NewJobGroupPage: NextPage = () => {
  const [name, setName] = useState("");
  const [description, setDescription] = useState("");

  const [fireBackButton, setFireBack] = useState(false);

  const { mutate } = api.jobGroups.create.useMutation({
    onSuccess: () => {
      setFireBack(true);
    },
    onError: (e) => {
  ...
  return (
    <>
      <Head>
        <title>New Binder - Valkyrie</title>
        <meta
          name="description"
...
export default NewJobGroupPage;
